...
class RGBColor extends Color_1.Color {
    ...
    getHSL() {
        ...
        let delta = cmax - cmin;
        let L = (cmax + cmin) / 2;
        let S = L === 0 ? 0 : L === 1 ? 0 : Color_1.Color.fixed(100 * (cmax - cmin) / (1 - Math.abs(2 * L - 1)));
        L = Color_1.Color.fixed(L * 100);
        let H = 0;
        if (cmax !== 0 && delta !== 0) {
            if (cmax === r) {
                H = ((g - b) / delta) % 6;
        ...
        H = Color_1.Color.fixed((H * 60 + 360) % 360);
        return { H, S, L };
    }
    getHSV() {
        let r = this.R;
        ...
        let cmin = Math.min(r, g, b);
        let delta = cmax - cmin;
        let V = Color_1.Color.fixed(cmax * 100);
        let S = cmax === 0 ? 0 : Color_1.Color.fixed(100 * delta / cmax);
        let H = 0;
        if (cmax !== 0 && delta !== 0) {
            if (cmax === r) {
                H = ((g - b) / delta) % 6;
        ...
        return { H: Color_1.Color.fixed((H * 60 + 360) % 360), S, V };
    }
    getRGB() {

...
const RGBColor_1 = require("./lib/RGBColor");
...
let compose = (operation, imgA, imgB) => {
    ...
    let tableA = new Frame_1.Frame(imgA.data, imgA.width, imgA.height); // ImageHelpers.imageData2pixels(imgA);
    let tableB = new Frame_1.Frame(imgB.data, imgB.width, imgB.height); // ImageHelpers.imageData2pixels(imgB);
    let result = new Frame_1.Frame(null, width, height);
    for (let row = 0; row < height; row++) {
        ...
        for (let col = 0; col < width; col++) {
            if (row < imgA.height && row < imgB.height && col < imgA.width && col < imgB.width) {
                ...
                let rgb = new RGBColor_1.RGBColor(color.getRGB());
                ...
                result.data[row][col] = new RGBColor_1.RGBColor(0, 0, 0);
